...
The renderer (``e2e_demo/templates/a2ui/a2ui_runner.html``) parses each line, builds
an id→object map, then mounts a DOM tree from ``children`` references (falling back
to ``parent``). Visual/behavioral fields live in ``props`` (e.g. ``props.text``,
...
- `children` (array of string): ids of child objects. **Primary** tree linkage.
- `parent` (string): id of the parent object. **Optional** alternative linkage
  (back-compat). If both `children` and `parent` are absent, the node is a root.
- `props` (object): all visual + behavioral properties. Supported keys:
  - text content: `text`, `label`, `title`
  - image: `src`
  - input/switch/slider: `value`, `min`, `max`, `step`, `checked`, `options`, `selectedIndex`, `placeholder`
  - theme: `primaryColor`, `className`
  - CSS-like: `width`, `height`, `backgroundColor`, `color`, `fontSize`, `fontWeight`,
    `borderRadius`, `padding`, `margin`, `flexDirection`, `justifyContent`, `alignItems`, `gap`, `flex`, `display`
...
- **Primary**: `children` (array of ids). The renderer `a2ui_runner.html` builds the
  tree from `children` references. This keeps each line independent and editable.
- **Optional**: `parent` (id string). Supported as a fallback for back-compat; if a
  node has neither `children` nor `parent`, it is treated as a root.
- The root object is the one whose `id` is not referenced by any other object's
  `children` (or, if an explicit `root` node exists, that node).
- `children` references are by id (string), not by inline nesting.
...
```
{"id":"root","type":"column","children":["header","body"],"props":{"padding":16,"gap":12,"backgroundColor":"#FFFFFF"}}
{"id":"header","type":"row","children":["title"],"props":{"justifyContent":"space-between"}}
{"id":"title","type":"text","children":[],"props":{"text":"My App","fontSize":24,"fontWeight":"700"}}
{"id":"body","type":"column","children":["card"],"props":{"gap":16}}
{"id":"card","type":"card","children":["label","input","btn"],"props":{"padding":16,"borderRadius":8,"gap":8}}
{"id":"label","type":"text","children":[],"props":{"text":"Email","fontSize":14,"fontWeight":"500"}}
{"id":"input","type":"input","children":[],"props":{"placeholder":"you@example.com"},"bind":{"source":"form","path":"email"}}
{"id":"btn","type":"button","children":[],"props":{"text":"Subscribe","onClick":"submit-form","backgroundColor":"#6366F1","color":"#FFFFFF","borderRadius":6,"padding":"8px 16px"}}
```
