...
sys.path.insert(0, r'C:\Code\screenshot-to-code\backend')
...
try:
    from backend.validate_code import validate_code, Stack
    ...
    try:
        ...
        print("validate_code imported (alt)")
    except Exception as e2:
        print(f"Alt import also failed: {e2}")
        sys.exit(1)

OUTPUT_DIR = Path(r'C:\Code\screenshot-to-code\e2e_demo\run_20260901')

# Stack name -> (filename, stack type)
stacks = [
...
for name, filename, stack_type in stacks:
    ...
    if not filepath.exists():
        print(f"{name:<20} {filename:<30} {'N/A':<8} {'file missing':<10}")
    ...
    try:
        ...
        if warnings:
            for warn in warnings[:3]:
                print(f"  WARN:  {warn}")
        results.append({"stack": name, "valid": valid, "errors": errors, "warnings": warnings})
        if not valid:
            all_valid = False
    except Exception as e:
